...
# Run one JazzyFS workload, render a clean post-workload sonification from the
# fresh decision log, then play that fresh WAV.
#
# Usage:
#   bash experiments/play_post_workload_sonification.sh [workload] [mode]
#
# Examples:
#   bash experiments/play_post_workload_sonification.sh phase_change adaptive
#   bash experiments/play_post_workload_sonification.sh random adaptive
...
RUN_LOG="/tmp/jazzyfs_post_workload_sonification.log"
OUTPUT_DIR="${OUTPUT_DIR:-$REPO/results/apfs/sonification/audio/Post workloads}"

WORKLOAD="${1:-phase_change}"
MODE="${2:-adaptive}"
...
RUN_LABEL="terminal_post_${MODE}_${WORKLOAD}"
OUT_WAV="$OUTPUT_DIR/${MODE}_${WORKLOAD}.wav"

reset_logs
mkdir -p "$MOUNT"
unmount_jazzyfs

echo "[JazzyFS] Starting $WORKLOAD in $MODE mode"
echo "[JazzyFS] Audio is off during the workload."
echo "[JazzyFS] A fresh polished WAV will be rendered and played after the run."
...
bash "$SCRIPT"
echo "[Workload] Finished. Rendering fresh polished sonification..."

"$PYTHON_BIN" "$REPO/experiments/render_post_workload_audio.py" \
...
echo "[Done] Fresh rendered playback complete."
