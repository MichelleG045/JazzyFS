...
- [`Experiments/result_summary.py`](/Users/michelleg/Projects/UCSC_Code/Masters_Thesis/jazzyfs/Experiments/result_summary.py): summarizes timing, decisions, and phase detection accuracy into CSV tables
- [`Experiments/plot_results.py`](/Users/michelleg/Projects/UCSC_Code/Masters_Thesis/jazzyfs/Experiments/plot_results.py): generates thesis figures from summary CSVs
- [`Experiments/run_all.sh`](/Users/michelleg/Projects/UCSC_Code/Masters_Thesis/jazzyfs/Experiments/run_all.sh): end-to-end pipeline — runs all experiments, summarizes, and plots
...
- [`workloads/synthetic/concurrent.sh`](/Users/michelleg/Projects/UCSC_Code/Masters_Thesis/jazzyfs/workloads/synthetic/concurrent.sh): parallel random reads from multiple processes
...
This runs all experiments, generates `timing_summary.csv`, `decision_summary.csv`, and `phase_accuracy_summary.csv`, then produces thesis figures in `results/{platform}/figures/`.

To run individual steps:

```bash
bash Experiments/run_experiments.sh          # access + decision logs (all modes)
bash Experiments/run_native_timing.sh        # native filesystem timing baseline
bash Experiments/run_jazzyfs_timing.sh none  # JazzyFS timing for one mode
bash Experiments/run_jazzyfs_timing.sh baseline
bash Experiments/run_jazzyfs_timing.sh adaptive
bash Experiments/run_depth_sweep.sh          # prefetch depth sweep
python3 Experiments/result_summary.py        # summarize to CSV
...
Experiment outputs are stored under [`results/`](/Users/michelleg/Projects/UCSC_Code/Masters_Thesis/jazzyfs/results):

- `results/{platform}/{workload}/{workload}_{mode}_run{N}/` — per-run access and decision logs
- `results/{platform}/native/` — timing CSVs for native and JazzyFS modes
- `results/{platform}/depth/` — timing CSVs for prefetch depth sweep
- `results/{platform}/timing_summary.csv` — aggregated timing with CI
- `results/{platform}/decision_summary.csv` — prefetch rate and confidence per workload/mode
- `results/{platform}/phase_accuracy_summary.csv` — phase detection accuracy vs ground truth
- `results/{platform}/figures/` — thesis figures generated by `plot_results.py`
